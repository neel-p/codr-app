...
import com.codr.aws.entity.*;
import com.codr.aws.exception.DuplicateEmailException;
...
public class AWSCognitoRepoImpl implements AWSCognitoRepo, CognitoResources {
    ...
    public AWSCognitoRepoImpl(Jdbi jdbi) {
        this.jdbi = jdbi;
    ...
    /**
     * <p>
     * Create a new user pool name.
     * </p>
     *
     * @param userPoolName The publicly visible user pool name
     * @return a CreateUserPoolResult object.
     */
    public CreateUserPoolResult createUserPool(String userPoolName) {
        PasswordPolicyType passwordPolicyType = new PasswordPolicyType().withTemporaryPasswordValidityDays(365).withMinimumLength(8).withRequireLowercase(true).withRequireUppercase(true).withRequireNumbers(true).withRequireSymbols(true);
        UserPoolPolicyType userPoolPolicyType = new UserPoolPolicyType().withPasswordPolicy(passwordPolicyType);

        CreateUserPoolRequest createUserPoolRequest = new CreateUserPoolRequest().withPoolName(userPoolName).withPolicies(userPoolPolicyType);
        CreateUserPoolResult createUserPoolResult = mIdentityProvider.createUserPool(createUserPoolRequest);
     ...
     * </p>
     *
     * @param userPoolClientName The publicly visible user pool client name
     * @return a CreateUserPoolClientResult object.
     */
    public CreateUserPoolClientResult createUserPoolClient(String userPoolClientName) {
        CreateUserPoolClientRequest createUserPoolClientRequest = new CreateUserPoolClientRequest().withUserPoolId(poolID).withClientName(userPoolClientName).withRefreshTokenValidity(30).withGenerateSecret(true);
        CreateUserPoolClientResult createUserPoolClientResult = mIdentityProvider.createUserPoolClient(createUserPoolClientRequest);
     ...
     * </P>
     *
     * @param userInformation The publicly visible user name
     *                        user's email address. This will be the email address that the temporary password will be sent to.
     *                        location     The users location (e.g., Bonaire, Aruba, Curacao)
     ...
     */
    @Override
    public void createNewUser(final UserInformation userInformation) throws AWSCognitoIdentityProviderException {
        final String emailAddr = userInformation.getEmailAddr();
        if (emailAddr != null && emailAddr.length() > 0) {
            // There should be no user with this email address, so info should be null
            UserInformation info = findUserByEmailAddr(emailAddr);
            if (info == null) {
                AdminCreateUserRequest cognitoRequest = new AdminCreateUserRequest()
                        .withUserPoolId(poolID)
                        .withUsername(userInformation.getUserName())
                        //.withTemporaryPassword("Neel1234@#")
                        ...
                        .withUserAttributes(
                                ...
                                //new AttributeType()
                                //       .withName(LOCATION)
                                //        .withValue(userInformation.getLocation()),
                                new AttributeType()
                ...
                // The AdminCreateUserResult returned by this function doesn't contain useful information so the
                // result is ignored.
                AdminCreateUserResult adminCreateUserResult = mIdentityProvider.adminCreateUser(cognitoRequest);
                System.out.println("adminCreateUserResult.getUser() ::: " + adminCreateUserResult.getUser().getUserStatus());
            } else {
     ...
     * </p>
     *
     * @param groupName   The publicly visible user pool group name
     * @param description The publicly visible user pool group description
     * @param roleArn     The publicly visible IAM Role name
     * @param precedence  The publicly visible user pool group precedence value
     * @return a CreateGroupResult object.
     */
    public void createNewUserGroup(String groupName, String description, String roleArn, Integer precedence) throws AWSCognitoIdentityProviderException {
        ...
        CreateGroupResult createGroupResult = mIdentityProvider.createGroup(createGroupRequest);
        System.out.println("createGroupResult.getGroup :::" + createGroupResult.getGroup());
    }
    ...
    public void deleteUser(final String userName, final String password) throws AWSCognitoIdentityProviderException {
        ...
        if (sessionInfo != null) {
            AdminDeleteUserRequest deleteRequest = new AdminDeleteUserRequest()
                    ...
                    .withUserPoolId(poolID);
            // the adminDeleteUserRequest returned an AdminDeleteUserResult which doesn't contain anything useful.
     ...
     */
    @Override
    public void updateUserAttributes(final UserInformation newInfo) throws AWSCognitoIdentityProviderException {
    ...
    protected SessionInfo sessionLogin(final String userName, final String password) throws AWSCognitoIdentityProviderException {
        ...
        try {
            ...
            InitiateAuthRequest authRequest = new InitiateAuthRequest()
                    ...
                    .addAuthParametersEntry("SECRET_HASH", clientID);
            //.addAuthParametersEntry("SRP_A", new AuthenticationHelper("eu-central-1XXXXXXXXX").getA().toString(16));
            //.withAuthParameters(authParams);
            InitiateAuthResult authResult = mIdentityProvider.initiateAuth(authRequest);
            ...
            if (authResult != null) {
                ...
                System.out.println("SessionInfo authResult is null..");
            }
        } catch (AWSCognitoIdentityProviderException idpe) {
            System.out.println("SessionInfo idpe Exception:: " + idpe.getMessage());
            idpe.printStackTrace();
        } catch (Exception e) {
            System.out.println("SessionInfo Exception:: " + e.getMessage());
            e.printStackTrace();
        }
        return info;
    }
     ...
     */
    @Override
    public LoginInformation userLogin(final String userName, final String password) throws AWSCognitoIdentityProviderException {
        ...
        if (sessionInfo != null) {
            ...
            loginInfo = new LoginInformation(userInformation, sessionInfo);
     ...
     */
    @Override
    public AdminRespondToAuthChallengeResult changeFromTemporaryPassword(final PasswordRequest passwordRequest) throws AWSCognitoIdentityProviderException {
        // Signin with the old/temporary password. Apparently this is needed to establish a session for the
        // password change.
        ...
        if (sessionString != null && sessionString.length() > 0) {
            ...
            challengeResponse = mIdentityProvider.adminRespondToAuthChallenge(changeRequest);
        }
     ...
     */
    @Override
    public UserInformation getUserInformation(final String userName) throws AWSCognitoIdentityProviderException {
        ...
        if (rsltUserName != null && emailAddr != null && location != null) {
            info = new UserInformation(rsltUserName, emailAddr, location);
        }
        return info;
    } // getUserInfo
     ...
     */
    @Override
    public UserInformation findUserByEmailAddr(String email) throws AWSCognitoIdentityProviderException {
        UserInformation info = null;
                ...
                if (users.size() == 1) {
                    ...
                    if (attributes != null) {
                        ...
                        if (userName != null && emailAddr != null && location != null) {
                            info = new UserInformation(userName, emailAddr, location);
                        }
                    }
                } else {
    ...
    public boolean hasUser(final String userName) {
        ...
        try {
            UserInformation info = getUserInformation(userName);
    ...
    /**
     * <p>
     * get all cognito users by pool id.
     * </p>
     *
     * @return List<UserType> objects by pool id.
     */
    public List<UserType> listUsers() throws Exception {
        ListUsersRequest listUsersRequest = new ListUsersRequest().withUserPoolId(poolID);
        ListUsersResult listUsersResult = mIdentityProvider.listUsers(listUsersRequest);
